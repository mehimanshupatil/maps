# Issues index

Workflow: fresh session per issue, pass `PRD.md` + the issue file.

| # | Issue | Status |
|---|---|---|
| 01 | Scaffold: app, terrain, basins, camera | ✅ Done (terrain later replaced by real DEM) |
| 02 | Data model: schema, real fill levels | ✅ Done |
| 03 | Interaction: hover, panels, header, status colors | ✅ Done |
| 04 | Flow: city, pipes, network topology | ✅ Done |
| 05 | Pipeline: OCR Action + Pages deploy | ✅ Done — LIVE at mumbai-lakes.himanshupatil.dev |
| 06 | History: backfill + sparklines | ✅ Done — 12 days backfilled, no gaps |
| 07 | Polish | 🟡 Partial — real-phone QA + perf pass remain |
| 08 | Intro flythrough | ✅ Done — ?intro=1 to force |
| 09 | Timeline scrubber | ✅ Done — play the season, ?date= links |
| 10 | Daily OG card refresh | ✅ Done — auto with each ingest |
| 11 | Three.js scene upgrade (water shader, sky, day-night, bloom, waterfalls, city lights) | ✅ Done — try ?t=23 |

Shipped beyond the original plan: real-DEM terrain + OSM rivers/reservoirs +
geoBoundaries districts, share button, "lasts till" date, fill-up intro
animation, data-driven rain, overflow shimmer, golden-hour grade, OG card,
labels toggle, CDP history blurbs (see `data/notes-bmc-cdp.md`).
